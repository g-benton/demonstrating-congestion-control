...
class BayesOptStrategy(SenderStrategy):
    ...
    def process_ack(self, serialized_ack: str) -> None:
        ...
        ## For now just leave all this the same and only mess with CWND ##
        ...
        if self.unacknowledged_packets.get(ack['seq_num']) is None:
            ...
            ############################################################################
            ## NOT OVERWRITING THE DUPLICATE ACK BRANCH BECAUSE I DON'T UNDERSTAND IT ##
            ############################################################################
        ...
        self.cwnds.append(int(self.cwnd))
